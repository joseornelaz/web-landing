...
const CardCarousel:React.FC<CardCarouselProps> = ({ items, type }) => {
  const [slidesToShowDesktop, setSlideToShowDesktop] = React.useState(3);

  // React.useEffect(() => {
  //   if(type === "prepa") {
  //     setSlideToShowDesktop();
  //   }
  // },[type]);
  ...
  const settings = {
    ...
    centerPadding: isMobile ? '0px' : '10px', // Espacio lateral cuando está centrado
  ...
  const getCardTestimonioStyles = (isActive: boolean) => (
        {
            p: 3,
            ...
            height: '300px', // Altura fija para mantener la consistencia
            // with: '402px',
            display: 'flex',
    ...
    <Box sx={{ width: '100%', maxWidth: 1200, margin: '0 auto', p: { xs: 0, md: 4 } }}>
      <Slider {...settings}>
        {items.map((item, index) => {
          ...
          const cardStyles = getCardTestimonioStyles(isActive);

          return (
            <Box key={index} sx={{ py: 4 }}> {/* Padding vertical para compensar el escalado */}
              <CardTestimonio item={item} cardStyles={cardStyles} />
            </Box>
          );
        })}
      </Slider>
